...
# replace install path, such that automator can find imguru command
sed -i '' "s|{INSTALL_PATH}|${IMGURU_INSTALL_PATH}|g" $AUTOMATOR_IMGUR_SCREENSHOT_UPLOAD_DIR$AUTOMATOR_DOCUMENT_FILE
sed -i '' "s|{INSTALL_PATH}|${IMGURU_INSTALL_PATH}|g" $AUTOMATOR_IMGUR_UPLOAD_DIR$AUTOMATOR_DOCUMENT_FILE
...
# open this, and install
open $AUTOMATOR_IMGUR_SCREENSHOT_UPLOAD_DIR
